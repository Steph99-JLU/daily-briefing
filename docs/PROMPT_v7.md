...
This supersedes the `briefing.py` system prompt approach (v6, gpt-4o). v7 is written
to run as an agentic Claude Code session with live Gmail search and web fetch, not as
a single completion call against a static prompt — it requires reading actual inbox
newsletters, crawling linked articles, and running narrow verification/enrichment
passes via sub-agents. `briefing.py`'s current pipeline (one `chat.completions.create`
call with no tools) cannot execute this spec; it has no inbox or web access and the
model would be generating the "news" from training data.

Running this daily therefore means either:
- a Claude Code session (interactive or scheduled) with Gmail + web-fetch tools, or
- rebuilding `briefing.py` around the Claude Agent SDK with real Gmail OAuth and a
  tool-use loop (new credentials, new hosting — a separate decision from this doc).
...
## 1. ACQUIRE
- Source: Gmail (stephan.klaas34@gmail.com). Inbox newsletters are the PRIMARY source — everything else is a cross-check, not a substitute.
